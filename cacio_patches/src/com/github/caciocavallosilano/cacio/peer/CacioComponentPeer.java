...
class CacioComponentPeer<AWTComponentType extends Component,
    ...
    public void setBounds(int x, int y, int width, int height, int op) {
        ...
        if (swingComponent != null) {
            ...
            // PATCHED FOR RuneLiteDroid: upstream calls swingComponent.validate()
            // here. validate() walks the Swing component tree and queues many
            // paint events onto the EDT, each allocating JNI local refs. Under
            // OpenJDK 21 for Android the JNI handle-list grow function
            // (libjvm.so+0xac44cc) has a use-after-free that fires under that
            // allocation burst — that's the SIGSEGV we've been chasing on every
            // agent-driven setBounds. Skipping validate() avoids the burst.
            // RuneLite/OSRS render the game canvas via BufferStrategy on a Canvas,
            // not via JScrollPane, so the missed validate doesn't matter for the
            // visible content. Top-level layout still validates naturally on
            // real paint events.
        }
    }
